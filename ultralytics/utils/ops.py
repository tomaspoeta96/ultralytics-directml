...
def crop_mask(masks, boxes):
    """
    Crops masks to bounding boxes, compatible with DirectML.
    ...
    Returns:
        torch.Tensor: Masks cropped to bounding boxes
    """
    n, h, w = masks.shape
    ...
    x1, y1, x2, y2 = boxes.to(torch.float32).chunk(4, 1)  # n,1 each
    
    # Create coordinate grids
    r = torch.arange(w, device=masks.device, dtype=torch.float32)  # w
    c = torch.arange(h, device=masks.device, dtype=torch.float32)  # h
    
    # Expand dimensions for broadcasting
    x1, y1, x2, y2 = [coord.view(n, 1, 1) for coord in [x1, y1, x2, y2]]
    r = r.view(1, 1, -1)  # 1,1,w
    c = c.view(1, -1, 1)  # 1,h,1
    
    # Create mask using logical operations
    r_mask = (r >= x1) & (r < x2)  # n,1,w
    c_mask = (c >= y1) & (c < y2)  # n,h,1
    
    # Combine masks using logical AND
    final_mask = r_mask & c_mask.expand(-1, -1, w)  # n,h,w
    
    # Convert boolean mask to float and multiply with input masks
    return masks * final_mask.to(masks.dtype)
